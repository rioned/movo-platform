        ...
        Column(Modifier.fillMaxSize()) {
            ...
            Box(Modifier.weight(1f)) {
                NavHost(nav, startDestination = "splash") {
                    ...
                    composable("review") { LaunchedEffect(Unit) { viewModel.quote(); viewModel.refreshNearbyRiders() }; ReviewScreen(state.draft, state.estimate, state.demo, state.busy, state.error, state.nearbyRiders, state.loadingRiders, viewModel::chooseRider, viewModel::refreshNearbyRiders, { paymentContinue = false; payment = true }, viewModel::quote, { viewModel.confirm { go("finding") } }, back) }
